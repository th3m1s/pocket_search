...
#        print self.html
  ...
  def grabArticle(self):
    for elem in self.html.findAll(True):
      ...
      unlikelyMatchString = []
      unlikelyMatchString.append(elem.get('id', 'N'))
      unlikelyMatchString.extend(elem.get('class', 'N'))

      # print(unlikelyMatchString)

      for unlikelyMatchString_i in unlikelyMatchString:
        if unlikelyMatchString_i is not 'N' and \
           self.regexps['unlikelyCandidates'].search(unlikelyMatchString_i) and \
           not self.regexps['okMaybeItsACandidate'].search(unlikelyMatchString_i) and \
            elem.name != 'body':
          #                print elem
          #                print '--------------------'
          elem.extract()
...
#                pass
      ...
      if elem.name == 'div':
        s = elem.renderContents()
        if not self.regexps['divToPElements'].search(str(s, encoding='utf-8')):
          elem.name = 'p'
...
#            print content
  ...
  def cleanArticle(self, content):
    ...
    self.clean(content, 'img')

    content = content.renderContents()

    content = self.regexps['killBreaks'].sub("<br />", str(content, encoding='utf-8'))
